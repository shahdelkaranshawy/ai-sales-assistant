...
Production recommendation code lives in `RecommendationGenerator` (`generator.py`): it adds full **expected_impact_aed / churn / NPS / win-probability** instruction tables, JSON-array retry prefixes, caching, and concurrency. See also `architecture/ai-action-recommender.pdf` for product context.
